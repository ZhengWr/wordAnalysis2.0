...
neo.models.Graph = (function() {
  ...
  Graph.prototype.nodes = function() {
    ...
    _ref = this.nodeMap;//_ref为各个点的信息  key为点的id
    _results = [];
    for (key in _ref) {
...
NeoD3Geometry = (function() {
  ...
  square = function(distance) {
    return distance * distance;//图中每个点的位置坐标
  };
  ...
  NeoD3Geometry.prototype.layoutRelationships = function(relationships) {
    ...
    for (_i = 0, _len = relationships.length; _i < _len; _i++) {
      ...
      length = Math.sqrt(square(dx) + square(dy));//线的长度
  ...
  NeoD3Geometry.prototype.setNodeRadii = function(nodes) {
    ...
    for (_i = 0, _len = nodes.length; _i < _len; _i++) {
      node = nodes[_i];
      _results.push(node.radius = parseFloat(this.style.forNode(node).get("diameter")) / 3);//设置点的半径
    }
    return _results;
  };
  ...
  _layout.force = function() {
    ...
    _force.init = function(render) {
      ...
      d3force = d3.layout.force()
                .linkDistance(function(d,i){return 140-count[i];})
                .charge(-1000)
                .gravity(0.3);
